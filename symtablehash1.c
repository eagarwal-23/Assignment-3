...
size_t bucketCounts[] = {509, 1021, 2039, 4093, 8191, 16381, 32749, 65521};
...
static void SymTable_rehash(SymTable_T symTable) {
   ...
   for (i = 0; i < oldNumBuckets; i++) {
      currentNode = symTable->bucketsArray[i];
...
int SymTable_put(SymTable_T symTable, const char* key,
   ...
   if ((symTable->numBindings > symTable->numBuckets) && (symTable->numBuckets < (int) MAX_BUCKETS)) {
